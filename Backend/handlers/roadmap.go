...
func GenerateRoadmap(c echo.Context) error {
	...
	if allowed, secondsLeft := rateLimiter.AllowRequest(userID, "research"); !allowed {
	...
	println("⚡ Calling Gemini API to generate NEW roadmap (with deduplication)...")
	roadmapJSON, err := utils.GenerateRoadmapWithDeduplication(preferenceHash, func() (string, error) {
		return utils.GenerateRoadmap(
	...
	if cacheCheckErr := config.DB.Where("preference_hash = ? AND roadmap_type = ?", preferenceHash, "research").First(&newCachedRoadmap).Error; cacheCheckErr == nil {
		println("✅ Found newly cached roadmap from concurrent request, using it")
